...
function loadEnvConfig () {
  ...
  if (get('SERVER_PORT') !== undefined) overlay.bot.port = parseIntSafe(get('SERVER_PORT'), undefined)
  if (get('BEDROCK_VERSION') !== undefined) overlay.bot.version = get('BEDROCK_VERSION')
  if (get('AUTH_OFFLINE') !== undefined) overlay.bot.offline = parseBoolean(get('AUTH_OFFLINE'), undefined)
  if (get('CONNECT_TIMEOUT_MS') !== undefined) overlay.bot.connectTimeoutMs = parseIntSafe(get('CONNECT_TIMEOUT_MS'), undefined)
  if (get('VIEW_DISTANCE') !== undefined) overlay.bot.viewDistance = parseIntSafe(get('VIEW_DISTANCE'), undefined)
  if (get('RAKNET_BACKEND') !== undefined) overlay.bot.raknetBackend = get('RAKNET_BACKEND')

  if (get('RECONNECT_ENABLED') !== undefined) overlay.reconnect.enabled = parseBoolean(get('RECONNECT_ENABLED'), undefined)
  if (get('RECONNECT_BASE_DELAY_MS') !== undefined) overlay.reconnect.baseDelayMs = parseIntSafe(get('RECONNECT_BASE_DELAY_MS'), undefined)
  if (get('RECONNECT_MAX_DELAY_MS') !== undefined) overlay.reconnect.maxDelayMs = parseIntSafe(get('RECONNECT_MAX_DELAY_MS'), undefined)
  if (get('RECONNECT_MAX_ATTEMPTS') !== undefined) overlay.reconnect.maxAttempts = parseIntSafe(get('RECONNECT_MAX_ATTEMPTS'), undefined)
  if (get('RECONNECT_JITTER_MS') !== undefined) overlay.reconnect.jitterMs = parseIntSafe(get('RECONNECT_JITTER_MS'), undefined)

  if (get('ANTI_AFK_ENABLED') !== undefined) overlay.antiAfk.enabled = parseBoolean(get('ANTI_AFK_ENABLED'), undefined)
  if (get('ANTI_AFK_MODE') !== undefined) overlay.antiAfk.mode = get('ANTI_AFK_MODE')
  if (get('ANTI_AFK_INTERVAL_MS') !== undefined) overlay.antiAfk.intervalMs = parseIntSafe(get('ANTI_AFK_INTERVAL_MS'), undefined)
  if (get('ANTI_AFK_WALK_RADIUS') !== undefined) overlay.antiAfk.walkRadius = parseFloatSafe(get('ANTI_AFK_WALK_RADIUS'), undefined)

  if (get('CHAT_HEARTBEAT_ENABLED') !== undefined) overlay.chatHeartbeat.enabled = parseBoolean(get('CHAT_HEARTBEAT_ENABLED'), undefined)
  if (get('CHAT_HEARTBEAT_INTERVAL_MS') !== undefined) overlay.chatHeartbeat.intervalMs = parseIntSafe(get('CHAT_HEARTBEAT_INTERVAL_MS'), undefined)
  ...
  if (get('HEALTH_CHECK_ENABLED') !== undefined) overlay.health.enabled = parseBoolean(get('HEALTH_CHECK_ENABLED'), undefined)
  if (get('HEALTH_CHECK_PORT') !== undefined) overlay.health.port = parseIntSafe(get('HEALTH_CHECK_PORT'), undefined)
 ...
 * instead of hitting them one at a time across repeated restarts. */
function validateConfig (config) {
  const errors = []

  if (!isPlausibleUsername(config.bot.username)) {
    errors.push('bot.username (BOT_USERNAME) must be a non-empty string of 32 characters or fewer.')
  }
  if (!isValidHost(config.bot.host)) {
    errors.push('bot.host (SERVER_HOST) is required and must be a bare hostname/IP, e.g. "myserver.aternos.me" (not a URL).')
  }
  if (!isValidPort(config.bot.port)) {
    errors.push('bot.port (SERVER_PORT) must be an integer between 1 and 65535 (Bedrock default is 19132).')
  }
  if (!isNonEmptyString(config.bot.version)) {
    errors.push('bot.version (BEDROCK_VERSION) must be a non-empty version string, e.g. "1.26.30".')
  }
  if (typeof config.bot.offline !== 'boolean') {
    errors.push('bot.offline (AUTH_OFFLINE) must resolve to true or false.')
  }
  if (!isPositiveInteger(config.bot.connectTimeoutMs)) {
    errors.push('bot.connectTimeoutMs (CONNECT_TIMEOUT_MS) must be a positive integer.')
  }
  if (!isPositiveInteger(config.bot.viewDistance) || config.bot.viewDistance > 32) {
    errors.push('bot.viewDistance (VIEW_DISTANCE) must be a positive integer no greater than 32.')
  }
  if (!isOneOf(config.bot.raknetBackend, RAKNET_BACKENDS)) {
    errors.push(`bot.raknetBackend (RAKNET_BACKEND) must be one of: ${RAKNET_BACKENDS.join(', ')}.`)
  }

  if (typeof config.reconnect.enabled !== 'boolean') {
    errors.push('reconnect.enabled (RECONNECT_ENABLED) must resolve to true or false.')
  }
  if (!isPositiveInteger(config.reconnect.baseDelayMs)) {
    errors.push('reconnect.baseDelayMs (RECONNECT_BASE_DELAY_MS) must be a positive integer.')
  }
  if (!isPositiveInteger(config.reconnect.maxDelayMs)) {
    errors.push('reconnect.maxDelayMs (RECONNECT_MAX_DELAY_MS) must be a positive integer.')
  }
  if (isPositiveInteger(config.reconnect.baseDelayMs) && isPositiveInteger(config.reconnect.maxDelayMs) &&
    ...
    errors.push('reconnect.baseDelayMs cannot be greater than reconnect.maxDelayMs.')
  }
  if (!isNonNegativeNumber(config.reconnect.maxAttempts) || !Number.isInteger(config.reconnect.maxAttempts)) {
    errors.push('reconnect.maxAttempts (RECONNECT_MAX_ATTEMPTS) must be 0 (unlimited) or a positive integer.')
  }
  if (!isNonNegativeNumber(config.reconnect.jitterMs)) {
    errors.push('reconnect.jitterMs (RECONNECT_JITTER_MS) must be 0 or a positive integer.')
  }

  if (typeof config.antiAfk.enabled !== 'boolean') {
    errors.push('antiAfk.enabled (ANTI_AFK_ENABLED) must resolve to true or false.')
  }
  if (!isOneOf(config.antiAfk.mode, ANTI_AFK_MODES)) {
    errors.push(`antiAfk.mode (ANTI_AFK_MODE) must be one of: ${ANTI_AFK_MODES.join(', ')}.`)
  }
  if (!isPositiveInteger(config.antiAfk.intervalMs) || config.antiAfk.intervalMs < 2000) {
    errors.push('antiAfk.intervalMs (ANTI_AFK_INTERVAL_MS) must be an integer >= 2000 (avoid spammy packet rates).')
  }
  if (!isNonNegativeNumber(config.antiAfk.walkRadius) || config.antiAfk.walkRadius > 3) {
    errors.push('antiAfk.walkRadius (ANTI_AFK_WALK_RADIUS) must be between 0 and 3 blocks.')
  }

  if (typeof config.chatHeartbeat.enabled !== 'boolean') {
    errors.push('chatHeartbeat.enabled (CHAT_HEARTBEAT_ENABLED) must resolve to true or false.')
  }
  if (!isPositiveInteger(config.chatHeartbeat.intervalMs) || config.chatHeartbeat.intervalMs < 30000) {
    errors.push('chatHeartbeat.intervalMs (CHAT_HEARTBEAT_INTERVAL_MS) must be an integer >= 30000 (avoid chat spam).')
  }
  if (!isNonEmptyString(config.chatHeartbeat.message)) {
    errors.push('chatHeartbeat.message (CHAT_HEARTBEAT_MESSAGE) must be a non-empty string.')
  }

  if (!isOneOf(config.logging.level, LOG_LEVELS)) {
    errors.push(`logging.level (LOG_LEVEL) must be one of: ${LOG_LEVELS.join(', ')}.`)
  }
  if (!isOneOf(config.logging.format, LOG_FORMATS)) {
    errors.push(`logging.format (LOG_FORMAT) must be one of: ${LOG_FORMATS.join(', ')}.`)
  }

  if (typeof config.health.enabled !== 'boolean') {
    errors.push('health.enabled (HEALTH_CHECK_ENABLED) must resolve to true or false.')
  }
  if (!isValidPort(config.health.port)) {
    errors.push('health.port (HEALTH_CHECK_PORT) must be an integer between 1 and 65535.')
  }
  if (!isNonEmptyString(config.health.path) || !config.health.path.startsWith('/')) {
    errors.push('health.path (HEALTH_CHECK_PATH) must be a non-empty path starting with "/".')
  }
